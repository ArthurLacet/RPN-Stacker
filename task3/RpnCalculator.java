...
public class RpnCalculator {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (scanner.hasNextLine()) {
            ...
            } else if (Regex.isPlus(input) || Regex.isMinus(input) || Regex.isMult(input) || Regex.isDiv(input)) {
                if (Regex.isPlus(input)){
                    Token token = new Token(TokenType.PLUS, input);
                    System.out.println(token.toString());
                }
                if (Regex.isMinus(input)){
                    Token token = new Token(TokenType.MINUS, input);
                    System.out.println(token.toString());
                }
                if (Regex.isMult(input)){
                    Token token = new Token(TokenType.STAR, input);
                    System.out.println(token.toString());

                }
                if (Regex.isDiv(input)){
